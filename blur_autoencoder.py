...
class BlurEncoder(tf.keras.layers.Layer):
    def __init__(self):
        super(BlurEncoder, self).__init__()
        self.encoder_conv_1 = tf.keras.layers.Conv2D(10, 3, (1, 1), padding='same',
                                                     kernel_initializer=tf.keras.initializers.RandomNormal(stddev=0.1),
                                                     bias_initializer=tf.keras.initializers.RandomNormal(stddev=0.1),
                                                     input_shape=(128, 128, 3))
        self.encoder_conv_2 = tf.keras.layers.Conv2D(10, 3, (1, 1), padding='same',
                                                     kernel_initializer=tf.keras.initializers.RandomNormal(stddev=0.1),
                                                     bias_initializer=tf.keras.initializers.RandomNormal(stddev=0.1))
        self.encoder_conv_3 = tf.keras.layers.Conv2D(10, 3, (1, 1), padding='same',
                                                     kernel_initializer=tf.keras.initializers.RandomNormal(stddev=0.1),
                                                     bias_initializer=tf.keras.initializers.RandomNormal(stddev=0.1))
    ...
    def call(self, images):
        ...
        shape = tf.shape(layer_1_output)
        layer_1_output = tf.image.resize(layer_1_output, (shape[1]/2, shape[2]/2), method='gaussian')
        layer_2_output = self.encoder_conv_2(layer_1_output)
        layer_2_output = tf.nn.leaky_relu(layer_2_output, 0.2)
        shape = tf.shape(layer_2_output)
        layer_2_output = tf.image.resize(layer_2_output, (shape[1]/2, shape[2]/2), method='gaussian')
        layer_3_output = self.encoder_conv_3(layer_2_output)
        shape = tf.shape(layer_3_output)
        layer_3_output = tf.image.resize(layer_3_output, (shape[1]/2, shape[2]/2), method='gaussian')
...
class BlurAutoEncoder(AutoEncoder):
    def __init__(self):
        super(BlurAutoEncoder, self).__init__()
        self.encoder = BlurEncoder()